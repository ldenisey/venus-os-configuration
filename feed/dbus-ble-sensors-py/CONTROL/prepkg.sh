...
wget -O "$SCRIPT_DIR/../opt/victronenergy/dbus-ble-sensors-py/ext/velib_python/settingsdevice.py" https://raw.githubusercontent.com/victronenergy/velib_python/refs/heads/master/settingsdevice.py

export SKIP_CYTHON=false; pip3 install bleak --no-deps --target "$SCRIPT_DIR/../opt/victronenergy/dbus-ble-sensors-py/ext/"
#pip3 install bleak --only-binary=:all: --target "$SCRIPT_DIR/../opt/victronenergy/dbus-ble-sensors-py/ext/"
